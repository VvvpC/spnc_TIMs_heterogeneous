...
# =========================================================
# 输入数据文件路径
CSV_FILE_PATH = 'saved_studies/TIMs_Uniform_tempsweep_Task_Pareto_20260107_185040_pareto_random_138.csv'
...
def create_configs_from_row(row):
    ...
    try:
        ...
        beta_prime = int(row['param_beta_prime'])
    ...
    params_config = Params(
        m0=m0,
        Nvirt=200,          # 固定为 200
        beta_prime=beta_prime,# 固定为 20.0
        params={
            'theta': theta,
            'gamma': gamma,
            'Nvirt': 200    # 确保字典中也更新
...
def main():
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Trials"):
        ...
        # 例如: Uniform_Pareto_ID_9
        task_name = f"Uniform_Pareto_ID_{trial_id}"
